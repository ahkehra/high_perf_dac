...
if $FILE1; then
  ...
  echo 1 > $FILE1
  echo "✓ Enabled high_perf_mode Excecuted on $(date +"%d-%m-%Y %r" )" >> $MODLOG
fi
# CPE Debug Mode
if $FILE2; then
  ...
  echo 1 > $FILE2
  echo "✓ Enabled cpe_debug_mode Excecuted on $(date +"%d-%m-%Y %r" )" >> $MODLOG
fi
# Impedance Detect EN
if $FILE3; then
  ...
  echo 1 > $FILE3
  echo "✓ Enabled impedance_detect_en Excecuted on $(date +"%d-%m-%Y %r" )" >> $MODLOG
fi
